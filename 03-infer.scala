...
// java -Xmx1024m -jar tmt-0.3.3.jar -Dscalanlp.distributed.hub=socket://s099082/hub -Dscalanlp.distributed.id=/tmt/8 edu.stanford.nlp.tmt.TMTMain "3-infer.scala"
...
val modelPath = file("lda-c2edf2b3-30-1c35a57d");
val model = LoadCVB0LDA(modelPath);

// A new dataset for inference.
val source = CSVFile("privacyfragments.csv") ~> IDColumn(1);
